...
class Saida extends RelatorioPdf
{
    ...
    public static function getRelatorioAllClientes($recibos, $analiseV){
        ...
        foreach($recibos as $recibo) {
            foreach ($recibo->analise as $analise) {
                ...
                $image1 = 'site/img/logo.jpg';
                $pdf->Cell( 20, $altura, '  '.$pdf->Image($image1, $pdf->GetX(), $pdf->GetY(), 4), 1, 0, 'C',0);
                $pdf->Ln();
                $y += $l;
                ...
                $diferenciada = $analise->pivot->orWhereIn('analise_id', [3])->count();
                $completa = $analise->pivot->orWhereIn('analise_id', [4])->count();
                //counts />
        ...
        $tot = ($quimica * $analiseV[0]['valor']) + ($fisica * $analiseV[1]['valor']) + ($diferenciada * $analiseV[2]['valor']) + ($diferenciada * $analiseV[3]['valor']);
